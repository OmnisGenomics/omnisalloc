...
* [dirty_decay_ms](https://jemalloc.net/jemalloc.3.html#opt.dirty_decay_ms) and
  [muzzy_decay_ms](https://jemalloc.net/jemalloc.3.html#opt.muzzy_decay_ms)
    ...
    call into jemalloc, unless you also enable `background_thread:true`.
    Without `background_thread:true`, processes that are sleeping
    (e.g. because they call `sleep()`, block on user input, network/file
    activity, or run subprocesses) will not purge memory.
